...
def visualizer(solution, puzzle_size):
    ...
    canvas = Canvas(master, width=canvas_width + 1, height=canvas_height + 1, bg=GUI_COLOR_2, borderwidth=0,
    ...
    if platform_system() is 'Darwin':
        system('''/usr/bin/osascript -e 'tell app "Finder" to set frontmost of process "''' + basename(
            executable) + '''" to true' ''')
    master.mainloop()
